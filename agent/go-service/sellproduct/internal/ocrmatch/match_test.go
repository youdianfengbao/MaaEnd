...
func TestFindBestMatchRejectsLongerCJKNames(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			ocr := []Item{{Text: c.ocrText, Box: maa.Rect{0, 0, 10, 10}}}
			got := FindBest(ocr, candidates)
			if got != nil {
...
func TestFindBestMatch_PremiumCandidateMatchesPremiumOCR(t *testing.T) {
	...
	ocr := []Item{{Text: "优质柑实罐头", Box: maa.Rect{0, 100, 10, 10}}}
	got := FindBest(ocr, candidates)
	if got == nil {
...
func TestFindBestMatch_NoPurpleCrystalCrossMatch(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			ocr := []Item{{Text: c.ocrText, Box: maa.Rect{0, 0, 10, 10}}}
			if got := FindBest(ocr, amethystBottleCands); got != nil {
...
func TestFindBestMatch_EnglishBracketsNoise(t *testing.T) {
	...
	ocr := []Item{{Text: "Canned Citrome [C]", Box: maa.Rect{0, 100, 10, 10}}}
	got := FindBest(ocr, candidates)
	if got == nil {
...
func TestFindBestMatch_EmptyCandidates(t *testing.T) {
	ocr := []Item{{Text: "any", Box: maa.Rect{}}}
	if got := FindBest(ocr, nil); got != nil {
		t.Errorf("expected nil, got %+v", got)
	}
}

func TestFindBestMatch_NoOCR(t *testing.T) {
	if got := FindBest(nil, []string{"紫晶质瓶"}); got != nil {
		t.Errorf("expected nil, got %+v", got)
	}
}
...
func TestFindBestMatchKeepsOperatorPrefixNoiseStrict(t *testing.T) {
	ocr := []Item{{Text: "大潘派", Box: maa.Rect{0, 0, 10, 10}}}
	if got := FindBest(ocr, []string{"大潘"}); got != nil {
...
func TestFindBestMatch_TierBEmptyDoesNotMatch(t *testing.T) {
	...
	ocr := []Item{{Text: "Canned Citrome D", Box: maa.Rect{0, 0, 10, 10}}}
	if got := FindBest(ocr, candidates); got != nil {
