...
func TestFindOperatorConflictSourceSupportsLocalizedPrompt(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			location, prompt, ok := findOperatorConflictSource([]ocrmatch.Item{{Text: test.text}}, data)
...
func TestFindOperatorConflictSourceProtectsUnknownLocation(t *testing.T) {
	...
	location, prompt, ok := findOperatorConflictSource([]ocrmatch.Item{{Text: text}}, data)
	if ok || location != "" {
...
func TestParseOperatorConflictParam(t *testing.T) {
	...
	if p.Result != operatorConflictResultManaged || p.Usage != operatorUsageTarget || p.Location != "RefugeeCamp" {
