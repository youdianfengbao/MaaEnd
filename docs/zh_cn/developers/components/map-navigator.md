...
- 路径点：先选择路径底图与层级；`加 NAVMESH/选择 (1)` 手工添加 `NAVMESH` 目标，也用于选中、拖拽点；`框选工具 (2)` 用于框选多个点。点击右上角详情中的「取消选择」、按 `Esc`，或在框选模式下单击空白地图均可取消选择；`Delete` 才会删除选中对象。实机录制得到的轨迹点保持 `RUN`。
