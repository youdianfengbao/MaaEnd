...
`model.mjs` determines whether to enter the pathfinding/photo-taking process based on whether the `routes.json` entry is complete. Unadapted points will directly take the `${Id}NotAdapted` branch. To make an observation point fully automated, you must provide the complete `EnterMap` (the actual `SceneEnterWorld*` node) / `MapName` / `MapAssert` / `CameraSwipeDirection` in `routes.json`, and select one from `MapPath` / `MapTarget` / `MapGoal`; when there is no available teleport point for the time being, you can omit the entry first, allowing it to run in the degraded "only accept and track" process.
