...
MaaEnd 使用三份由 zmdmap 数据 CI 生成并发布的精简游戏数据：
...
这些文件都是从 TableCfg 中按任务需要裁剪出的游戏数据，不是完整的 TableCfg 快照，也不包含数据生产端的版本或来源元数据。MaaEnd 通过 `data/version.txt` 单独记录当前数据版本，`fetch-data.mjs` 根据 zmdmap 版本接口下载三份文件；版本未变化且本地缓存完整时跳过下载，网络检查失败时保留当前缓存。
...
完整生成某个任务时使用对应的 pnpm 命令。三个命令都会先同步 zmdmap 精简游戏数据，再执行当前任务自己的生成链路：
...
`data/scripts/` 中的三个 Python 入口供 zmdmap 数据 CI 使用，不是 MaaEnd 日常生成流程的一部分。它们彼此独立，只读取当前任务需要的 TableCfg，并输出对应的精简游戏数据：
