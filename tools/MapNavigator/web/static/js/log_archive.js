...
export function groupLogInputFiles(inputFiles) {
  ...
  for (const file of Array.from(inputFiles || [])) {
    ...
    if (identity.part !== null && group.files.some((entry) => archiveIdentity(entry).part === identity.part)) {
  ...
  for (const group of groups.values()) {
    ...
    if (parts.length) {
      ...
      if (missing.length) {
        throw new Error(
          `${group.label} 缺少分包：${missing.map((part) => `part${String(part).padStart(2, "0")}`).join("、")}`,
        );
      }
    }
  }
...
export async function openZipArchive(blob, sourceName = "archive.zip") {
  ...
  if (disk !== 0 || centralDisk !== 0)
    throw new Error(`${sourceName} 是传统跨卷 ZIP，当前仅支持 MaaEnd 独立 part 分包`);
