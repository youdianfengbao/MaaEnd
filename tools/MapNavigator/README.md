...
4. 需要从作者路径以外的位置测试时，点击地图右上角的 `设置规划起点` 并在地图取点；再点击 `规划当前片段` 查看 C++ 运行时展开的路线。需要评估滑索时，先勾选 `启用滑索规划（桌面端）`。
5. 点击 `复制路径`；启用滑索后按钮会改为 `复制完整参数`，并一并导出 `zip: true`。

`Esc` 只取消当前选择或临时取点工具，不删除数据；`Delete` 删除选中的作者路点，`清除规划` 只移除预览，不会修改作者路径。
